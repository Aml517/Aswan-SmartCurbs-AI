...
class AnnotationTool:
    ...
    def _assign_class_to_pending(self, class_name):
        ...
        if self.pending_box is not None:
            x1, y1, x2, y2 = self.pending_box   # normalise so x1<x2 and y1<y2 whatever the drag direction
...
def main():
    ...
    video_path = str(path(CFG["video"]))
    output_path = str(path("outputs/ground_truth.json"))
    
    # Selection of distributed frames for a statistically sound evaluation
    evaluation_frames = [0, 150, 300, 450, 600, 740]   # 0-based frame numbers
